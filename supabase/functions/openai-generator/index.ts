...
const handleAction = async (actionData, context, user, supabaseAdmin) => {
  const { action } = actionData;
  const { userList, rawProjects } = context;
  ...
  try {
    ...
    switch (feature) {
      case 'analyze-projects': {
        ...
        const context = { userList, rawProjects: projects, rawGoals: goals };
        // --- End Fetch Context ---
...
`;
        ...
        try {
            ...
            const actionResult = await handleAction(actionData, context, user, supabaseAdmin);
