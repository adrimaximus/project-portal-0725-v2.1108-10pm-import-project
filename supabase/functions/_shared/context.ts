...
export const buildContext = async (supabaseClient, user) => {
  ...
  try {
    ...
    console.error("[DIAGNOSTIC] buildContext: CRITICAL UNHANDLED ERROR during context build:", error);
    // Return an empty context to prevent a full crash, but log the critical error.
    return {
