...
const TagsPropertiesPage = () => {
  ...
  const { data: properties = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from('custom_properties').select('*').eq('category', 'tag').order('label');
  ...
  const handleSave = async (propertyData: SavedPropertyFormValues) => {
    ...
    if (error) {
      ...
      queryClient.invalidateQueries({ queryKey: ['custom_properties', 'tag'] });
      setIsFormOpen(false);
    }
  };
      ...
      <div className="space-y-6">
        <Breadcrumb>
          <BreadcrumbList>
            ...
            <BreadcrumbSeparator />
            <BreadcrumbItem><BreadcrumbPage>Tag Properties</BreadcrumbPage></BreadcrumbItem>
          </BreadcrumbList>
        </Breadcrumb>
        
        <div className="flex justify-between items-center">
          <div>
            <h1 className="text-2xl font-bold tracking-tight">Tag Properties</h1>
            <p className="text-muted-foreground">Modify and create tag properties.</p>
          </div>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Create and manage custom fields for your tags.</CardDescription>
          </CardHeader>
          <CardContent>
            <Table>
              <TableHeader>
...
export default TagsPropertiesPage;