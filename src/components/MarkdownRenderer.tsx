...
const MarkdownRenderer: React.FC<MarkdownRendererProps> = ({ children, className, replyContext, onReplyClick }) => {
  // Pre-process mentions: @[Name](id) -> [@Name](mention:id)
  const processedContent = children.replace(/@\[(.*?)\]\((.*?)\)/g, '[@$1](mention:$2)');

  const formattedTime = replyContext?.timestamp 
    ? formatDistanceToNow(new Date(replyContext.timestamp), { addSuffix: true }) 
    : '';
  ...
  return (
    <div className={cn("markdown-content text-sm leading-relaxed", className)}>
      {replyContext && (
        <blockquote
          onClick={onReplyClick ? onReplyClick : undefined}
          className={cn(
            "border-l-4 border-primary/40 pl-3 py-1 my-2 bg-muted/30 rounded-r-sm text-xs cursor-pointer hover:bg-muted/50 transition-colors select-none",
            !onReplyClick && "cursor-default"
          )}
        >
          <div className="flex items-center gap-2 mb-0.5">
            <strong className="font-semibold text-primary">{replyContext.senderName}</strong>
            {formattedTime && <span className="text-muted-foreground/70 text-[10px]">· {formattedTime}</span>}
          </div>
          <div className="text-muted-foreground/90 line-clamp-2">
            {replyContext.isDeleted ? (
