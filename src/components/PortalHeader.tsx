...
const PortalHeader = ({ summary }: PortalHeaderProps) => {
  ...
  return (
    <>
      <header className="sticky top-0 z-50 flex h-14 items-center gap-2 border-b bg-background px-2 sm:gap-4 sm:px-4 lg:h-[60px] lg:px-6">
        <Sheet>
            ...
            <DropdownMenuContent align="end" className="w-80 sm:w-96 max-h-[80vh] overflow-y-auto">
              ...
              {unreadNotifications.length > 0 ? (
                ...
                <div className="p-4 text-center text-sm text-muted-foreground">
                  {notifications.length > 0 ? "You're all caught up!" : "You have no notifications."}
                </div>
              )}
              <DropdownMenuSeparator />
