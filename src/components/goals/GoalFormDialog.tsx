...
const GoalFormDialog = ({ open, onOpenChange, onGoalUpdate, goal }: GoalFormDialogProps) => {
  ...
  async function onSubmit(values: z.infer<typeof goalFormSchema>) {
    ...
    if (isNewGoal) {
      if (!apiKey) {
        toast.error("API Key not set. Please go to Settings to add it.", {
      ...
      try {
        const prompt = `${values.title}${values.description ? `: ${values.description}` : ''}`;
        ...
        if (!data.imageUrl) throw new Error("AI service did not return an image.");

        icon = data.imageUrl;
        toast.success("Icon generated successfully!", { id: toastId });
      } catch (error: any) {
        console.error("Icon generation failed:", error);
        toast.error(`Icon generation failed: ${error.message}. Using default icon.`, { id: toastId });
      }
    }
