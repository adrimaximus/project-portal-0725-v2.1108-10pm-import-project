...
// Handle both 'notes' (DB column) and 'note' keys
// State for the existing note
// Mention state
...
// Dependent on goal.id so it refreshes if goal changes
// 5 minutes
...
const GoalYearlyProgress = (
    {
        goal,
        onToggleCompletion,
        onUpdateCompletion
    }: GoalYearlyProgressProps
) => {
    const {
    ...
    const months = Array.from({
        length: 12
    }).map((_, i) => startOfMonth(new Date(displayYear, i, 1)));
    ...
    const monthlyData = months.map(monthDate => {
        ...
        const completionMap = new Map<string, {
            ...
            note?: string;
        }>(relevantCompletions.map(c => [format(parseISO(c.date), "yyyy-MM-dd"), {
    ...
    const {
        ...
        `).eq("goal_id", goal.id).eq("comment_date", dateStr).order("created_at", {
                ascending: true
            });
    ...
    const {
        ...
        queryFn: async () => {
            ...
            const {
                data,
                error
            } = await supabase.from("profiles").select("id, first_name, last_name, email, avatar_url").in("id", collaboratorIds).eq("status", "active");
    ...
    const addCommentMutation = useMutation({
        mutationFn: async (
            ...
            if (!selectedDay || !user)
                throw new Error("No day selected or user not logged in");
    ...
    const updateCommentMutation = useMutation({
        mutationFn: async (
            ...
            const {
                error
            } = await supabase.from("goal_comments").update({
                content,
    ...
    const handleDayClick = (
        ...
        if (isAfter(day.date, todayStart))
            return;
        ...
        if (day.attachmentUrl) {
            setExistingAttachment({
                url: day.attachmentUrl,
                name: day.attachmentName || "Attachment"
            });
        } else {
    ...
    const renderCommentContent = (content: string) => {
        ...
        while ((match = mentionRegex.exec(content)) !== null) {
            ...
            parts.push(<span
                key={match.index}
                className="font-medium text-primary hover:underline cursor-pointer">@{match[1]}
            </span>);
        ...
        <>
            <Card>
                <CardHeader>
                    ...
                    <CardDescription>You have completed this target <strong>{totalCompleted}</strong>of <strong>{totalPossible}</strong>times in {displayYear}.
                                  </CardDescription>
                        ...
                        return (
                            <div
                                ...
                                <div className="grid grid-cols-7 gap-1">
                                    {Array.from({
                                        length: (month.days[0].date.getDay() + 6) % 7
                                                ...
                                                <Tooltip>
                                                    <TooltipTrigger asChild>
                                                        <button
                                                            ...
                                                            style={buttonStyle}>
                                                            {(day.attachmentUrl || day.note) && (<div
                                                                className="absolute inset-0 flex items-center justify-center pointer-events-none">
                                                                <div
                                                                    className="w-1 h-1 bg-white rounded-full shadow-[0_0_2px_rgba(0,0,0,0.5)]" />
                                                            </div>)}
                                                        </button>
                                                    </TooltipTrigger>
                                                    <TooltipContent>
                                                        <p>{format(day.date, "PPP", {
                                                                locale: enUS
                                                            })}</p>
                                                        {isFutureDay ? <p>Future date</p> : !isValidDay ? <p>Not a scheduled day</p> : day.isCompleted !== undefined ? (<>
            ...
            <Dialog open={!!selectedDay} onOpenChange={open => !open && setSelectedDay(null)}>
                <DialogContent
                    className="sm:max-w-[400px] p-0 overflow-hidden border-none shadow-xl flex flex-col max-h-[90vh]">
                    {}
                    ...
                    <div className="p-6 space-y-6 overflow-y-auto">
                        {}
                        <div
                            className="flex flex-row items-center justify-between p-4 rounded-xl border bg-card/50 hover:bg-card hover:border-primary/20 transition-all duration-200">
                            <div className="space-y-1 flex-1 mr-4">
                                <Label
                                    htmlFor="completed-toggle"
                                    className="text-base font-semibold cursor-pointer">Completion Status
                                                              </Label>
                        ...
                        <div className="space-y-3">
                            ...
                            {existingAttachment && !file ? (<div
                                ...
                                className="group relative flex items-center gap-3 p-3 rounded-xl border bg-card hover:bg-accent/5 transition-all duration-200">
                                <div
                                    className="h-10 w-10 rounded-lg bg-primary/10 flex items-center justify-center text-primary shrink-0">
                                ...
                                <div
                                    className="h-12 w-12 rounded-full bg-muted group-hover:bg-primary/10 flex items-center justify-center transition-colors duration-200">
                                    <Paperclip className="h-6 w-6 text-muted-foreground group-hover:text-primary" />
                                </div>
                                <div className="text-center space-y-1">
                                    <p
                                        className="text-sm font-medium group-hover:text-primary transition-colors">Click to upload</p>
                        ...
                        <div className="space-y-3 pt-4 border-t">
                            ...
                            <div className="space-y-4">
                                {isLoadingComments ? (<div className="flex justify-center p-4">
                                ...
                                </div>) : comments?.length === 0 ? (<p className="text-xs text-muted-foreground text-center py-2">No comments yet.</p>) : (comments?.map((comment: any) => (<div key={comment.id} className="flex gap-3 text-sm">
                                    ...
                                    <div className="flex-1 space-y-1">
                                        ...
                                        {editingCommentId === comment.id ? (<div className="flex flex-col gap-2">
                                            <Textarea
                                                value={editContent}
                                                onChange={e => setEditContent(e.target.value)}
                                                className="text-xs min-h-[60px]" />
                                            {editFile && (<div
                                                className="flex items-center gap-2 p-2 bg-muted rounded-md text-xs border">
                                        ...
                                        </div>) : (<div className="p-2 bg-muted/50 rounded-lg text-xs break-words relative group">
                                            {renderCommentContent(comment.content)}
                                            {comment.attachments_jsonb && comment.attachments_jsonb.length > 0 && (<div className="mt-2 space-y-1 pt-1 border-t border-border/50">
                                                {comment.attachments_jsonb.map((att: any, idx: number) => (<div key={idx}>
                                                    {att.type?.startsWith("image/") ? (<div
                                                        className="mt-2 mb-1 rounded-md overflow-hidden border border-border/50 bg-background max-w-[240px]">
                                                        ...
                                                        className="flex items-center gap-2 p-1.5 bg-background border rounded-md max-w-fit hover:bg-accent/50 transition-colors">
                                                        <Paperclip className="h-3 w-3 text-muted-foreground" />
                                                        <a
                                                            href={att.url}
                                                            target="_blank"
                                                            rel="noopener noreferrer"
                                                            className="text-xs hover:underline truncate max-w-[150px] text-primary">
                                                            {att.name}
                                                        </a>
                                                    </div>)}
                                                </div>))}
                                            </div>)}
                                            {user?.id === comment.user_id && (<div
                    ...
                    <DialogFooter className="p-6 pt-0 shrink-0">
                        <div className="grid grid-cols-2 gap-3 w-full">
                            <Button
                                ...
                                className="h-11 rounded-lg">Cancel
                                                      </Button>
                            <Button
                                onClick={handleSaveDay}
                                className="h-11 rounded-lg bg-primary text-primary-foreground hover:bg-primary/90 shadow-sm">Save Changes
                                                      </Button>
                        </div>
                    </DialogFooter>
                </DialogContent>
            ...
            <AlertDialog open={!!dayToConfirm} onOpenChange={() => setDayToConfirm(null)}>
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>This action will change the completion status for a past date.
                                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
                        <AlertDialogCancel asChild>
                            <Button variant="outline" onClick={() => setDayToConfirm(null)}>Cancel
                                              </Button>
                        </AlertDialogCancel>
                        <AlertDialogAction asChild>
                            <Button onClick={handleConfirmToggle}>Confirm
                                              </Button>
                        </AlertDialogAction>
                    </AlertDialogFooter>
                </AlertDialogContent>
