...
const PageEditorDialog = ({ open, onOpenChange, folders = [], folder, article, onSuccess }: PageEditorDialogProps) => {
  ...
  const isContentEmpty = !contentValue || contentValue.trim() === '<p><br></p>' || contentValue.trim().length < 15;
  ...
  const handleGenerateArticle = async () => {
    ...
    try {
      const { data, error } = await supabase.functions.invoke('ai-handler', {
        body: { feature: 'generate-article-from-title', payload: { title } }
      });
            ...
            <FormField control={form.control} name="content" render={({ field }) => (
              <FormItem>
                <div className="flex justify-between items-center">
                  ...
                  {isContentEmpty ? (
                    <Button type="button" variant="outline" size="sm" onClick={handleGenerateArticle} disabled={isGenerating || !titleValue.trim()}>
