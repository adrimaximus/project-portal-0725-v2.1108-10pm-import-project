    ...
    <Card className="group hover:bg-muted/50 transition-colors" style={{ borderLeft: `4px solid ${folder.color || '#6b7280'}` }}>
      <CardContent className="p-3">
        <div className="flex items-start justify-between">
          <Link to={`/knowledge-base/folders/${folder.slug}`} className="flex-1 min-w-0">
          ...
          {onEdit && onDelete && (
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="ghost" size="icon" className="h-8 w-8 flex-shrink-0 opacity-0 group-hover:opacity-100 -mt-1 -mr-1">
