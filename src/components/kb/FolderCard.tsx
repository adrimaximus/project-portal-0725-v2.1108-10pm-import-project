...
const FolderCard = ({ folder, onEdit, onDelete }: FolderCardProps) => {
  ...
  return (
    <Card className="hover:shadow-md transition-shadow h-full flex flex-col" style={{ borderTop: `4px solid ${folder.color || '#6b7280'}` }}>
      <CardHeader className="flex-row items-start justify-between">
        <Link to={`/knowledge-base/folders/${folder.slug}`} className="flex items-center gap-3 group flex-1 min-w-0">
