...
const ProjectBrief = (
                ...
                <CardContent className="p-4">
                    <div
                        ...
                        <div className="flex flex-col items-center gap-2 text-muted-foreground w-full">
                            ...
                            {isUploading ? (<div className="w-full max-w-[240px] space-y-2">
                            ...
                            </div>) : isDragActive ? (<p>Drop the files here ...</p>) : (<p>Drag 'n' drop files here, or click to select</p>)}
                        </div>
                    </div>
                    <div className="mt-4 space-y-2">
                        {files && files.map(file => (<div
                            ...
                            <div className="flex items-center gap-2 flex-shrink-0">
                                ...
                                <Button
                                    variant="ghost"
                                    size="icon"
                                    onClick={() => onFileDelete(file.storage_path)}
                                    className="h-6 w-6">
    ...
    return (
        <div
            ...
            {files && files.length > 0 ? (<div className="space-y-2">
                {files.map(file => (<div
                    key={file.id || file.name}
                    className="flex items-center gap-3 p-2 bg-background rounded-md text-sm">
                    <FileText className="h-4 w-4 flex-shrink-0" />
                    <a
                        href={file.url}
                        target="_blank"
            ...
            </div>) : (<div className="text-center flex items-center justify-center h-full pt-4">
