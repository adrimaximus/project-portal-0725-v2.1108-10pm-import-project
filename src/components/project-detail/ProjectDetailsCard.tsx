...
const ProjectDetailsCard = (
    ...
    const {
        ...
        queryFn: async () => {
            const {
                data,
                error
            } = await supabase.from("people").select("*").order("full_name", {
                ascending: true
            });
    ...
    const updateClientMutation = useMutation({
        mutationFn: async (value: string) => {
            if (value.startsWith("company-")) {
                ...
                const {
                    error: peopleError
                } = await supabase.from("people_projects").delete().eq("project_id", project.id);
    ...
    const renderDateRange = () => {
        ...
        const isExclusiveEndDate = project.due_date && end.getUTCHours() === 0 && end.getUTCMinutes() === 0 && end.getUTCSeconds() === 0 && end.getUTCMilliseconds() === 0 && !isSameDay(start, end);
    ...
    const renderClientSelectContent = () => (<SelectContent className="max-h-72">
        <SelectGroup>
            <SelectItem
                value="create-new"
        ...
        <SelectSeparator />
        {isLoadingPeople || isLoadingCompanies ? (<SelectItem value="loading" disabled>Loading...</SelectItem>) : (<>
            {allCompanies && allCompanies.length > 0 && (<SelectGroup>
                <SelectLabel>Companies</SelectLabel>
                {allCompanies.map(
                    company => (<SelectItem key={`company-${company.id}`} value={`company-${company.id}`}>
                        {company.name}
                    </SelectItem>)
                )}
            </SelectGroup>)}
            {allPeople && allPeople.length > 0 && (<SelectGroup>
                <SelectLabel>People</SelectLabel>
                {allPeople.map(person => (<SelectItem key={person.id} value={person.id}>
        ...
        <Card>
                        ...
                        <div className="space-y-6">
                            <div className="flex items-start gap-4">
                                ...
                                <div className="w-full">
                                    ...
                                    {isEditing ? (<DatePickerWithRange
                                        ...
                                        className="w-full" />) : (<p className="text-muted-foreground">
                                        {renderDateRange()}
                                    </p>)}
                                </div>
                            </div>
                            {hasPermission("project:budget:read") && (<div className="flex items-start gap-4">
                                <Wallet className="h-4 w-4 mt-1 flex-shrink-0 text-muted-foreground" />
                                <div>
                                    <p className="font-medium">Budget</p>
                                    {isEditing ? (<CurrencyInput
                            ...
                            <div className="flex items-start gap-4">
                                ...
                                <div className="w-full">
                                    ...
                                    {isEditing ? (<AddressAutocompleteInput
                                        value={project.venue || ""}
                                        onChange={value => onFieldChange("venue", value)} />) : (renderVenue())}
                                </div>
                            </div>
                            <div className="flex items-start gap-4">
                            ...
                            <div className="flex items-start gap-4">
                                ...
                                <div className="w-full">
                                    ...
                                    {isEditing ? (<Select
                                        ...
                                        <SelectContent>
                                            {projectStatuses.length > 0 ? (projectStatuses.map(status => (<SelectItem
                                                key={status.id}
                                                value={status.name}
                                                ...
                                                <div className="flex items-center">
                                                    <StatusBadge
                                                        status={status.name}
                                                        className="border-0 px-0 bg-transparent text-foreground hover:bg-transparent" />
                                                </div>
                                            </SelectItem>))) : (PROJECT_STATUS_OPTIONS.map(option => (<SelectItem
                                                key={option.value}
                                                value={option.value}
                            ...
                            <div className="flex items-start gap-4">
                                ...
                                <div className="w-full">
                                    ...
                                    {isEditing ? (<Select
                                        ...
                                        <SelectContent>
                                            {isLoadingPaymentStatuses ? (<SelectItem value="loading" disabled>Loading...</SelectItem>) : (paymentStatuses.map(option => (<SelectItem key={option.id} value={option.name}>
                                                <div className="flex items-center gap-2">
                                                    <span
                                        ...
                                        <Select
                                            ...
                                            <SelectTrigger
                                                className="w-full h-auto p-0 border-none bg-transparent focus:ring-0 shadow-none hover:bg-accent/50 rounded-md transition-colors text-left flex items-center justify-between group">
                                                <Badge
                                                    variant="outline"
                                                    className={cn(
                                                        "font-normal border-transparent cursor-pointer hover:opacity-80 transition-opacity"
                                                    )}
                                                    style={{
                                ...
                                <div className="w-full">
                                    ...
                                    {isEditing ? (<Select
                                        ...
                                        <Select
                                            ...
                                            <SelectTrigger
                                                className="w-full h-auto p-0 border-none bg-transparent focus:ring-0 shadow-none hover:bg-accent/50 rounded-md transition-colors text-left flex items-center justify-between group">
                                                {clientInfo.name ? (<div className="flex items-center gap-3 py-1 px-1">
                                                    {clientInfo.logoUrl ? (<img
                                                        src={clientInfo.logoUrl}
                                                        alt={clientInfo.companyName || ""}
                                                        className="h-8 w-8 object-contain rounded-md bg-muted p-1" />) : clientInfo.avatarUrl ? (<Avatar className="h-8 w-8">
                                                        <AvatarImage src={clientInfo.avatarUrl} />
                                                        <AvatarFallback><User className="h-4 w-4" /></AvatarFallback>
                                                    </Avatar>) : (<div className="h-8 w-8 rounded-md bg-muted flex items-center justify-center">
                                                        <Building className="h-4 w-4 text-muted-foreground" />
                                                    </div>)}
                                                    <div>
                                                        <p className="text-foreground font-semibold text-sm">{clientInfo.name}</p>
                                                        {clientInfo.companyName && clientInfo.companyName !== clientInfo.name && (<p className="text-muted-foreground text-xs">{clientInfo.companyName}</p>)}
                                                    </div>
                                                </div>) : (<p className="text-muted-foreground py-1 px-1">No client assigned</p>)}
                                            </SelectTrigger>
                                            {renderClientSelectContent()}
                                        </Select>
                                    </div>)}
            ...
            <Dialog open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen}>
                <DialogContent>
                    ...
                    <Tabs
                        defaultValue="person"
                        onValueChange={v => setNewClientType(v as "person" | "company")}>
