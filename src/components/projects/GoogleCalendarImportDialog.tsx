...
export const GoogleCalendarImportDialog = (
    {
        open,
        onOpenChange,
        onImport,
    ...
    const {
        ...
        queryFn: async () => {
            const {
                data,
                error
            } = await supabase.functions.invoke("get-google-calendar-events");
    ...
    const {
        ...
        queryFn: async () => {
            const {
                data,
                error
            } = await supabase.from("projects").select("origin_event_id");
    ...
    const filteredEvents = useMemo(() => {
        if (isOverallLoading || events.length === 0)
            return [];
    ...
    const handleSelectEvent = (eventId: string) => {
        setSelectedEvents(
            prev => prev.includes(eventId) ? prev.filter(id => id !== eventId) : [...prev, eventId]
        );
    };
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="sm:max-w-[625px] rounded-[40px]">
                <DialogHeader>
                ...
                <div className="relative h-96">
                    ...
                    {error && (<div className="text-destructive text-center p-4">{error.message}</div>)}
                    {!isOverallLoading && !error && filteredEvents.length === 0 && (<div className="text-center p-4 text-muted-foreground">No upcoming events found to import.</div>)}
                    {!isOverallLoading && !error && filteredEvents.length > 0 && (<div className="h-full flex flex-col border rounded-md">
                        <div
                            className="flex items-center justify-between p-3 border-b bg-muted/50 flex-shrink-0">
                ...
                <DialogFooter>
                    ...
                    <Button
                        ...
                        {isImporting && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}Import {selectedEvents.length > 0 ? `${selectedEvents.length} Event(s)` : ""}
                    </Button>
                </DialogFooter>
            </DialogContent>
        </Dialog>
